...
| # | 区域名称 | 位置 | 注册表组件 | 包含元素 |
|---|---------|------|-----------|---------|
| 1 | 统计指标行（如有） | 并排-N | `MetricCard` × N | 每个卡片含数字和标签，点击切换筛选 |
| 2 | `{如：搜索筛选操作区}` | 顶部 | `FilterBar` | **同行左右布局**：左侧 `.filter-left` 放筛选控件 + 查询/重置按钮，右侧 `.filter-right` 放操作按钮(如 发起工单)。控件含：关键词输入框 + 模板下拉 + 优先级下拉 + SLA状态下拉 + 日期范围选择器(快捷选项:今天/本周/本月，默认近30天)。注意：实例状态筛选由统计卡片覆盖，不重复放置状态下拉 |
| 3 | `{如：数据表格}` | 全宽 | `DataTable` | `{如：复选框列 + 状态标签列(StatusTag) + 名称 + 时间 + 操作列(fixed right)}` |
