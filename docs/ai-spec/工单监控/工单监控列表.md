...
| # | 区域名称 | 位置 | 注册表组件 | 包含元素 |
|---|---------|------|-----------|---------|
| 1 | 统计指标行 | 并排-4 | `MetricCard` × 4 | 全部(总数) + 草稿 + 进行中 + 已关闭，每卡片含数字和标签，hover 可点击筛选，计数由后端 `WorkOrderStats` 驱动 |
| 2 | 搜索筛选区 | 顶部 | `FilterBar` | 关键词输入框(placeholder="工单编号/发起人") + 模板名称下拉单选 + 优先级下拉单选(紧急/高/普通/低) + SLA状态下拉单选(正常/预警/超时) + 创建时间日期范围选择器(快捷选项:今天/本周/本月，默认近30天) + 查询按钮 + 重置按钮。实例状态筛选已由统计卡片点击覆盖，不重复放置状态下拉; 与操作按钮区 同行顶部居左 |
...
| 4 | 数据表格 | 全宽 | `DataTable` | 列：工单编号(link，点击打开抽屉) + 模板名称 + 工单进度(如`3/5`，来自`activePathIndex/activePathNodeCount`) + 当前节点(currentNodeName，已关闭显示"—") + 实例状态(StatusTag，草稿/进行中/已关闭) + 优先级(StatusTag) + SLA状态(自定义红黄绿圆点+剩余时间文案，hover 弹出 TTR/TTS 明细 Tooltip) + 当前处理人 + 发起人 + 创建时间 + 操作列(详情按钮, fixed right)。空数据文案："暂无工单实例" |
...
| `status` | `InstanceStatus` | 实例状态：`draft` / `active` / `closed`，见 §3.2 |
| `priority` | `Priority` | 优先级：`urgent` / `high` / `normal` / `low`（工单固定字段，非表单字段） |
...
| 枚举值 | 标签文字 | 颜色类型 | 判定逻辑 |
|--------|---------|---------|---------|
| `draft` | 草稿 | `info` | 所有节点均为 `pending` |
| `active` | 进行中 | `warning` | 部分节点已开始但非全部完成 |
| `closed` | 已关闭 | `normal` | 所有节点为 `completed` 或 `skipped` |

> 进行中的具体阶段（指派/处置/验收等）通过 `currentNodeName` 列展示，由模板配置决定。
...
| `all` | `number` | 全部工单数 |
| `draft` | `number` | 草稿数 |
| `active` | `number` | 进行中工单数 |
| `closed` | `number` | 已关闭数 |

> stats 接口始终返回 4 个字段（含零值），前端据此动态渲染 4 张统计卡片，不得硬编码卡片列表。
...
| `priority` | 优先级 | `string` | `select` | — | `urgent/high/normal/low` | — |
...
stateDiagram-v2
    ...
    进行中 --> 已关闭 : 全部节点完成
    进行中 --> 已关闭 : 取消
    note right of 进行中: 具体阶段由currentNodeName体现\n(如"指派维修人员""现场处置""物业验收")
```
...
- 统计卡片始终展示 4 张（全部 + 3 种 InstanceStatus），计数为 0 时正常显示 0
...
|---------|-----------|----------------------|
| 统计指标行 | `MetricCard` × 4 | 4 个指标卡片并排：label=全部/草稿/进行中/已关闭，value=对应数字。@click 切换筛选 |
