...
| 设计元素 | 组件 | 来源 | 关键 Props |
|---------|------|------|-----------|
| 状态标签 | StatusTag | `components/business/StatusTag.vue` | `status="plan_prepare\|plan_review\|plan_trial\|plan_production"` |
...
| 文件 | 行数 | 说明 |
|------|:---:|------|
| `src/types/resumption.ts` | 134 | `ResumptionPlanItem`、`ResumptionQuery`、`PaginatedData`、`STEP_META`、`STAGES` |
| `src/api/adapters/resumption-dao.ts` | 260 | `getResumptionPlanList`、`createResumptionPlan`、`getStageLabel`、`getManagementUnits`、`updateStep` |
| `src/stores/resumption.ts` | 41 | `useResumptionStore`：list/loading/query/total/fetchList/search |
| `src/views/resumption/PlanList.vue` | 338 | 本页面 |
| `src/components/business/StatusTag.vue` | — | 追加 6 个复工状态映射 |
...
| 2026-07-20 | v1.1：复工场所改为关联管理单元（el-select + allow-create），字段从 workshopName 改为 locationName |
