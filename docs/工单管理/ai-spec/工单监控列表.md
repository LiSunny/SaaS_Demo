...
| # | 区域名称 | 位置 | 注册表组件 | 包含元素 |
|---|---------|------|-----------|---------|
| 1 | 统计指标行 | 并排-4 | `MetricCard` × 4 | 全部 + 草稿 + 进行中 + 已关闭，每卡片含数字和标签，hover 可点击筛选，计数由后端 `WorkOrderStats` 驱动 |
| 2 | 搜索筛选区 | 顶部 | `FilterBar` | 工单编号/发起人搜索框 + 优先级下拉(紧急/高/普通/低) + SLA状态下拉(正常/超时) + 时间日期范围选择器 + 查询按钮(primary)。与操作按钮区同行 |
| 3 | 操作按钮 | 顶部 | 自定义 | **发起工单**(outlined primary, +图标) —— 快捷入口，实际弹窗逻辑复用工作台的 CreateOrderDialog |
| 4 | 数据表格 | 全宽 | `DataTable` | 列：工单状态(StatusTag: 草稿=blue/进行中=orange/已关闭=grey) + SLA(Tag+icon: 超时=red+icon/正常=green+icon) + 工单编号 + 工单标题 + 优先级(Tag+icon: 紧急red/高orange/普通yellow/低blue) + 流程进度(进度条+X/N数字) + 发起时间(可排序) + 操作列(仅预览图标, fixed right)。空数据文案："暂无工单实例" |
