...
---
> 版本：v1.5 | 日期：2026-06-26 | 状态：已确认
> 上游文档：[平台总业务设计](../biz-design.md) · [平台岗位设计](../平台岗位设计.md) · [租户管理业务设计](../租户管理/biz-design.md)
...
> 按[平台管理角色（维度 A）](../租户管理/biz-design.md#32-维度-a平台管理角色)分类梳理，不同管理角色在企业链中的位置不同→用户管理的规模和复杂度也不同。
...
- **岗位即权限**：在每家企业内，分配岗位 = 同时获得该企业内的数据范围和工作台配置
...
3. **平台管理员权限**：可跨企业查看用户列表（只读），不可新增/编辑/删除。可查看用户关联了哪些企业
4. **岗位分配**：在企业内部，一个用户可分配多个岗位，数据范围取岗位并集；同一用户在不同企业可担任不同岗位
...
| 数据范围 | 该岗位的数据可见范围（self/assigned/service/all） |
| 权限集 | 该岗位拥有的操作权限列表 |
...
| 用户 ID | 关联 User | |
| 企业 ID | 关联 Enterprise | |
| 岗位 | 多选 | 在该企业内分配的岗位（从 11 个平台岗位中选择） |
...
| # | 决策 | 结论 | 确认日期 |
|---|------|------|---------|
| 1 | 一人多岗 vs 一岗 | **一人多岗**，数据范围取岗位并集 | 2026-06-26 |
