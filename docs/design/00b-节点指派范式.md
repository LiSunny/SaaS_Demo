...
流程中的 `assign`（指派）节点和 `confirm`（审批）节点都需要回答同一个问题：
...
```
FlowNode {
  assignSource: 'static' | 'dynamic'   // 指派来源，默认 static
  assignConfig?: {                     // 静态指派配置
...
|------|------|------|
| **决定时机** | 模板配置时 | 工单运行时 |
| **决定者** | 模板设计者 | 工单当前操作者（如预警方管理员） |
| **灵活性** | 低，修改需更新模板版本 | 高，每次工单可指不同人 |
| **适用** | 审核人固定的企业内流程 | 接收方自行分派的跨企业流程 |
| **配置复杂度** | 低，选人即可 | 需先设计表单字段 |
| **运维成本** | 人员变动需更新模板 | 无，运行时效即可 |
| **模板可移植性** | 差，绑死具体人/组织 | 好，模板可跨企业复用 |
