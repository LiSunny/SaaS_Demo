...
| 查看详情 | 点击工单进入详情页 | 查看完整流程、处置记录和当前节点操作 | ⭐ 未关闭工单需区分只读信息与可编辑操作区 | 双栏布局：进度+只读信息+当前节点操作+流转记录（已更新） |
...
| 详情 | WorkOrderDetail（双栏布局：流程进度+只读信息+当前节点操作+流转记录时间线+SLA指标） |
...
> 设计决策日期：2026-06-09。核心结论：**"我的工单"与"工单监控"是两个不同的产品**。
...
| **入口** | 工作台 `MyTasksWidget` | 独立页面 `/system/monitor` |
