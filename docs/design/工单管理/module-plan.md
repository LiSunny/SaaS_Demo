...
- **核心功能点**：
  ...
  - [x] 独立详情页（已关闭工单完整查看）
  - [ ] 详情抽屉展示 DynamicForm（未关闭时可编辑，已关闭时只读）
...
- **核心功能点**：
  ...
  - [x] 优先级 + 描述（已有）
  - [ ] 嵌入 DynamicForm：渲染模板发起节点的可编辑字段
  - [ ] 表单数据提交后写入 formData
  - [ ] 提交创建（状态=草稿/待指派）
- **关键页面**：发起弹窗（el-dialog，CreateOrderDialog 升级）
- **依赖模块**：M0、M1、M-N（DynamicForm）
- **复杂度评估**：中
- **Demo 优先级**：P0

### M3 处置页面 + 移动端处置 新增 P0
- **职责**：Web 端处置页面（执行/验收）+ 微信小程序处置
- **Web 端核心功能点**：
  - [ ] 处置页面 `/system/order/:id/process`（工单上下文 + DynamicForm）
  - [ ] 执行/验收复用同一路由，按 nodeType 切换表单权限
  - [ ] DynamicForm 渲染全部字段（可编辑的写，其余只读）
  - [ ] 表单提交 → nodeRecords + formData 更新
- **移动端核心功能点**：
  - [ ] 待办列表（待接单+处置中）
  - [ ] 处置表单（DynamicForm 小程序版）
- **关键页面**：`views/workflow/Process.vue` + 小程序页面
