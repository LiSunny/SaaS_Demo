# 工单监控 接口规格

---

## 1. 获取工单实例列表

**描述**：分页查询工单实例列表，支持多维度筛选
**方法**：GET
**路径**：`/api/v1/work-order/instances`
**权限**：需要登录

### 请求参数

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `page` | number | 否 | 页码，默认 1 |
| `pageSize` | number | 否 | 每页条数，默认 20，最大 100 |
| `keyword` | string | 否 | 关键词，模糊匹配工单编号、发起人姓名 |
| `status` | string | 否 | 实例状态，多选用逗号分隔。可选值：draft / pending_assign / pending_accept / processing / verifying / closed |
| `templateId` | number | 否 | 模板 ID，精确匹配 |
| `priority` | string | 否 | 优先级：urgent / normal / low |
| `slaStatus` | string | 否 | SLA 状态：normal / warning / timeout |
| `startDate` | string | 否 | 创建时间起始，格式 YYYY-MM-DD |
| `endDate` | string | 否 | 创建时间截止，格式 YYYY-MM-DD |
| `sortBy` | string | 否 | 排序字段，默认 createdAt |
| `sortOrder` | string | 否 | 排序方向，asc / desc，默认 desc |

### 请求示例

```
GET /api/v1/work-order/instances?page=1&pageSize=20&status=processing,verifying&slaStatus=warning&startDate=2026-05-01&endDate=2026-06-01
```

### 响应结构

```json
{
  "code": 0,
  "message": "success",
  "data": {
    "total": 128,
    "page": 1,
    "pageSize": 20,
    "stats": {
      "all": 128,
      "draft": 3,
      "pendingAssign": 12,
      "pendingAccept": 8,
      "processing": 45,
      "verifying": 18,
      "closed": 42
    },
    "list": [
      {
        "id": 1,
        "orderNo": "WO20260601-001",
        "templateId": 1,
        "templateName": "设备维修工单模板",
        "templateVersion": 3,
        "status": "processing",
        "priority": "urgent",
        "currentNodeId": 103,
        "currentNodeName": "执行节点",
        "currentNodeType": "execute",
        "currentAssigneeId": 15,
        "currentAssigneeName": "李四",
        "creatorId": 10,
        "creatorName": "张三",
        "creatorOrgId": 1001,
        "creatorOrgName": "xxx物业管理有限公司",
        "parentOrderId": null,
        "createdAt": "2026-06-01 09:30:00",
        "updatedAt": "2026-06-01 10:45:00",
        "sla": {
          "ttrMinutes": 120,
          "ttsMinutes": 1440,
          "ttrStartedAt": "2026-06-01 09:35:00",
          "ttrEndedAt": "2026-06-01 09:42:00",
          "ttsStartedAt": "2026-06-01 09:42:00",
          "ttsPausedAt": null,
          "yellowThreshold": 0.8,
          "ttrProgress": null,
          "ttsProgress": 0.72,
          "slaStatus": "warning"
        }
      },
      {
        "id": 2,
        "orderNo": "WO20260601-002",
        "templateId": 2,
        "templateName": "安全生产督办流程",
        "templateVersion": 1,
        "status": "verifying",
        "priority": "normal",
        "currentNodeId": 204,
        "currentNodeName": "确认节点",
        "currentNodeType": "confirm",
        "currentAssigneeId": 10,
        "currentAssigneeName": "张三",
        "creatorId": 10,
        "creatorName": "张三",
        "creatorOrgId": 1001,
        "creatorOrgName": "xxx物业管理有限公司",
        "parentOrderId": null,
        "createdAt": "2026-06-01 08:15:00",
        "updatedAt": "2026-06-01 11:00:00",
        "sla": {
          "ttrMinutes": null,
          "ttsMinutes": 480,
          "ttrStartedAt": null,
          "ttrEndedAt": null,
          "ttsStartedAt": "2026-06-01 08:15:00",
          "ttsPausedAt": "2026-06-01 10:30:00",
          "yellowThreshold": 0.8,
          "ttrProgress": null,
          "ttsProgress": 0.35,
          "slaStatus": "normal"
        }
      }
    ]
  }
}
```

### 错误码

| code | 说明 | 处理建议 |
|------|------|---------|
| 1001 | 参数校验失败 | 检查必填参数和参数格式 |
| 1002 | 无权限访问 | 确认登录状态和角色权限 |

---

## 2. 获取工单实例详情

**描述**：获取单个工单实例的完整信息，包含节点链路和处理记录
**方法**：GET
**路径**：`/api/v1/work-order/instances/:id`
**权限**：需要登录

### 请求参数

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `id` | number | 是 | 工单实例 ID（路径参数） |

### 请求示例

```
GET /api/v1/work-order/instances/1
```

### 响应结构

```json
{
  "code": 0,
  "message": "success",
  "data": {
    "id": 1,
    "orderNo": "WO20260601-001",
    "templateId": 1,
    "templateName": "设备维修工单模板",
    "templateVersion": 3,
    "status": "processing",
    "priority": "urgent",
    "currentNodeId": 103,
    "currentNodeName": "执行节点",
    "currentNodeType": "execute",
    "currentAssigneeId": 15,
    "currentAssigneeName": "李四",
    "creatorId": 10,
    "creatorName": "张三",
    "creatorOrgId": 1001,
    "creatorOrgName": "xxx物业管理有限公司",
    "parentOrderId": null,
    "createdAt": "2026-06-01 09:30:00",
    "updatedAt": "2026-06-01 10:45:00",
    "closedAt": null,
    "closedBy": null,
    "sla": {
      "ttrMinutes": 120,
      "ttsMinutes": 1440,
      "ttrStartedAt": "2026-06-01 09:35:00",
      "ttrEndedAt": "2026-06-01 09:42:00",
      "ttsStartedAt": "2026-06-01 09:42:00",
      "ttsPausedAt": null,
      "yellowThreshold": 0.8,
      "ttrProgress": null,
      "ttsProgress": 0.72,
      "slaStatus": "warning"
    },
    "nodes": [
      {
        "id": 101,
        "name": "发起节点",
        "type": "start",
        "status": "completed",
        "assigneeName": "张三",
        "completedAt": "2026-06-01 09:30:00",
        "order": 1
      },
      {
        "id": 102,
        "name": "指派节点",
        "type": "assign",
        "status": "completed",
        "assigneeName": "系统",
        "completedAt": "2026-06-01 09:35:00",
        "order": 2
      },
      {
        "id": 103,
        "name": "执行节点",
        "type": "execute",
        "status": "in_progress",
        "assigneeName": "李四",
        "completedAt": null,
        "order": 3
      },
      {
        "id": 104,
        "name": "确认节点",
        "type": "confirm",
        "status": "pending",
        "assigneeName": "张三",
        "completedAt": null,
        "order": 4
      },
      {
        "id": 105,
        "name": "关闭节点",
        "type": "close",
        "status": "pending",
        "assigneeName": null,
        "completedAt": null,
        "order": 5
      }
    ],
    "records": [
      {
        "id": 301,
        "action": "接单",
        "operatorName": "李四",
        "operatorOrgName": "xxx物业管理有限公司",
        "content": "确认接单，开始处理",
        "createdAt": "2026-06-01 09:42:00"
      },
      {
        "id": 302,
        "action": "指派",
        "operatorName": "系统",
        "operatorOrgName": null,
        "content": "自动指派给维修一部 - 李四",
        "createdAt": "2026-06-01 09:35:00"
      },
      {
        "id": 303,
        "action": "创建工单",
        "operatorName": "张三",
        "operatorOrgName": "xxx物业管理有限公司",
        "content": "创建设备维修工单",
        "createdAt": "2026-06-01 09:30:00"
      }
    ]
  }
}
```

### 错误码

| code | 说明 | 处理建议 |
|------|------|---------|
| 2001 | 工单实例不存在 | 检查 ID 是否正确 |
| 1002 | 无权限访问 | 确认登录状态和角色权限 |

---

## 3. 获取工单统计卡片

**描述**：获取按实例状态分组的统计计数，用于顶部统计卡片
**方法**：GET
**路径**：`/api/v1/work-order/instances/stats`
**权限**：需要登录

### 请求参数

无额外参数（自动根据当前用户角色过滤可见范围）。

### 请求示例

```
GET /api/v1/work-order/instances/stats
```

### 响应结构

```json
{
  "code": 0,
  "message": "success",
  "data": {
    "all": 128,
    "draft": 3,
    "pendingAssign": 12,
    "pendingAccept": 8,
    "processing": 45,
    "verifying": 18,
    "closed": 42
  }
}
```

### 错误码

| code | 说明 | 处理建议 |
|------|------|---------|
| 1002 | 无权限访问 | 确认登录状态和角色权限 |

---

## 4. 取消工单

**描述**：手动取消工单实例，取消后进入"已关闭"状态但不触发归档回调
**方法**：POST
**路径**：`/api/v1/work-order/instances/:id/cancel`
**权限**：需要平台运营方角色

### 请求参数

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `reason` | string | 是 | 取消原因，最多 200 字 |

### 请求示例

```json
{
  "reason": "工单重复创建，需取消"
}
```

### 响应结构

```json
{
  "code": 0,
  "message": "工单 WO20260601-001 已取消",
  "data": {
    "id": 1,
    "orderNo": "WO20260601-001",
    "status": "closed",
    "closedAt": "2026-06-01 14:30:00",
    "closedBy": "平台管理员"
  }
}
```

### 错误码

| code | 说明 | 处理建议 |
|------|------|---------|
| 3001 | 工单实例不存在 | 检查 ID 是否正确 |
| 3002 | 工单已关闭，不可取消 | 提示用户工单已关闭 |
| 3003 | 无操作权限 | 仅平台运营方可取消 |
| 3004 | 取消原因不能为空 | 提示用户填写原因 |

---

## 5. 强制改派

**描述**：平台运营方强制更改工单当前节点的处理人
**方法**：POST
**路径**：`/api/v1/work-order/instances/:id/reassign`
**权限**：需要平台运营方角色

### 请求参数

| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `targetAssigneeId` | number | 是 | 目标处理人 ID |
| `reason` | string | 是 | 改派原因，最多 200 字 |

### 请求示例

```json
{
  "targetAssigneeId": 20,
  "reason": "原处理人李四请假，改派给王五"
}
```

### 响应结构

```json
{
  "code": 0,
  "message": "工单 WO20260601-001 已改派给王五",
  "data": {
    "id": 1,
    "orderNo": "WO20260601-001",
    "previousAssigneeId": 15,
    "previousAssigneeName": "李四",
    "currentAssigneeId": 20,
    "currentAssigneeName": "王五",
    "updatedAt": "2026-06-01 14:35:00"
  }
}
```

### 错误码

| code | 说明 | 处理建议 |
|------|------|---------|
| 4001 | 工单实例不存在 | 检查 ID 是否正确 |
| 4002 | 工单已关闭，不可改派 | 提示用户工单已关闭 |
| 4003 | 无操作权限 | 仅平台运营方可强制改派 |
| 4004 | 目标处理人不存在 | 检查人员 ID 是否正确 |
| 4005 | 改派原因不能为空 | 提示用户填写原因 |

---

## 附：Mock 数据设计要点

1. **Mock 列表**至少包含 15-20 条工单，覆盖所有状态和 SLA 三色情况
2. **状态分布**建议：处置中 > 已关闭 > 验收中 > 待指派 > 待接单 > 草稿
3. **SLA 分布**建议：至少包含 2 条超时（红）、3 条预警（黄）
4. **优先级分布**建议：普通 50%、紧急 30%、低优 20%
5. **时间戳**使用相对当前时间计算，确保 SLA 进度动态变化可展示
